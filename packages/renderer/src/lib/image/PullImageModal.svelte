  ...
  if (!value.includes('/')) {
    ...
    for (const registry of preferredRegistries) {
      try {
        ...
        const searchResultItems = await window.searchImageInRegistry(options);
    ...
    const searchResultItems = await window.searchImageInRegistry(options);
    return searchResultItems.map(r => {
...
<Dialog title="Pull image from a registry" onclose={closeCallback}>
  {#snippet content()}
    <div class="space-y-4">
      <div class="self-center text-sm text-[var(--pd-table-body-text)]">
        Specify preferred registries for pulling images in
        <Link on:click={gotoManageRegistries}>Settings &gt; Registries</Link>.
        <Button class="ml-2" on:click={gotoManageRegistries} icon={faCog} type="link">Manage registries</Button>
      </div>
      ...
      <div>
        ...
        {#if selectedProviderConnection?.type === 'podman' && podmanFQN}
          <Checkbox class="pt-2" bind:checked={usePodmanFQN} title="Use Podman FQN" disabled={podmanFQN === ''}>
            Use Podman FQN for shortname image
          </Checkbox>
        {/if}
        {#if imageNameInvalid}
      ...
      <div class="h-[185px]" hidden={initTerminal === false}>
        <TerminalWindow class="h-full" bind:terminal={logsPull} disableStdIn />
      </div>
    </div>
  {/snippet}
  ...
  {#snippet buttons()}
    {#if !pullFinished}
      <Button type="link" on:click={closeCallback} disabled={pullInProgress}>Cancel</Button>
      <Button
    ...
    {:else}
      <Button type="primary" on:click={closeCallback}>Done</Button>
    {/if}
  {/snippet}
</Dialog>
