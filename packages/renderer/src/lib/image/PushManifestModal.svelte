...
onMount(async () => {
  if (manifestInfoToPush.isManifest) {
    try {
      manifestDetails = await window.inspectManifest(manifestInfoToPush.engineId, manifestInfoToPush.id);
      console.log('manifest details:', manifestDetails);
    } catch (err) {
      console.error(err);
    }
  }
...
async function pushManifest() {
  ...
  console.log('pushing manifest:', manifestInfoToPush);
  try {
