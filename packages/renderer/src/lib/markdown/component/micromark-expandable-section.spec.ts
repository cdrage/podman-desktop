...
test('Expect createExpandableSection to return a button with an expandable section', async () => {
  ...
  expect(htmlSanitized).toContain(
    `<button class='flex space-x-2 text-[var(--pd-link)] w-fit hover:bg-white hover:bg-opacity-10 text-xs items-center' data-expandable='micromark-expandable-1'`,
  );
  expect(htmlSanitized).toContain(
...
test('Expect createExpandableSection to return a button and a visible expandable section', async () => {
  ...
  const htmlSanitized = html.replace('\n', '').replace(/\s+/g, ' ');
  expect(htmlSanitized).toContain(
    `<button class='flex space-x-2 text-[var(--pd-link)] w-fit hover:bg-white hover:bg-opacity-10 text-xs items-center' data-expandable='micromark-expandable-2'`,
  );
  expect(htmlSanitized).toContain(
...
test('Expect createExpandableSection to return a hidden button and expandable section', async () => {
  ...
  const htmlSanitized = html.replace('\n', '').replace(/\s+/g, ' ');
  expect(htmlSanitized).toContain(
    `<button class='flex space-x-2 text-[var(--pd-link)] w-fit hover:bg-white hover:bg-opacity-10 text-xs items-center' data-expandable='micromark-expandable-3' style='display: none;'`,
  );
  expect(htmlSanitized).toContain(
