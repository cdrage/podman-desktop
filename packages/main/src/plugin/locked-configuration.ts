...
export class LockedConfiguration {
  ...
  protected getManagedLockedFile(): string {
    if (isMac()) {
      return SYSTEM_LOCKED_FILE_MAC;
    } else if (isWindows()) {
      const programData = process.env['PROGRAMDATA'] ?? 'C:\\ProgramData';
      return path.join(programData, SYSTEM_LOCKED_FILE_WINDOWS_DIR, SYSTEM_LOCKED_FILE_WINDOWS_FILE);
    } else if (isLinux()) {
      return SYSTEM_LOCKED_FILE_LINUX;
    }
  ...
  public async getContent(): Promise<{ [key: string]: unknown }> {
    // "Create" the managed locked file
    const managedLockedFile = this.getManagedLockedFile();
