...
describe('handle exec without shell (security)', () => {
  ...
  test('handle exec with special characters in arguments', async () => {
    const extensionName = 'myExtension';
    const mockProcess = createMockSpawnProcess();
    ...
    expect(spawnMock).toHaveBeenCalledTimes(1);
    const spawnCall = spawnMock.mock.calls[0]!;
    expect(spawnCall[1]).toEqual(['run', '--name', 'test$container', '--env', 'VAR=value;other']);
    expect(spawnCall[2]!.shell).toBeUndefined();

    expect(result.code).toBe(0);
  });

  test('handle exec with launcher and filtered docker args', async () => {
    const extensionName = 'myExtension';
    const mockProcess = createMockSpawnProcess();

    spawnMock.mockReturnValue(mockProcess);

    const execPromise = dockerPluginAdapter.handleExec(extensionName, 'podman', 'run', [
      '-v',
    ...
    // Verify docker socket is filtered out and launcher is used
    expect(spawnMock).toHaveBeenCalledTimes(1);
    const spawnCall = spawnMock.mock.calls[0]!;
    expect(spawnCall[0]).toBe('podman');
    expect(spawnCall[1]).toEqual(['run', '--format', '{{.Status}}']);
    expect(spawnCall[2]!.shell).toBeUndefined();
