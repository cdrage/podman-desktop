...
export class GithubProvider extends podmanDesktopAPI.CliToolProvider {
  ...
  // TODO / UPDATE.
  // TEMPORARY: This is a temporary function to get the latest release and see if it has an update
  // Must add tests + caveats (ignore RC versions... draft... etc.)
  async hasUpdate(url: URL, localVersion: string): Promise<boolean> {
    const { organization, repo } = this.parseUrl(url);
    ...
    return localVersion !== latestRelease.tag_name;
  }
}
