...
async function handleComposeBinaryToggle(
  ...
  // If the user somehow set the toggle to false and the binary is installed, show a warning message and set the toggle back to true
  // this is because uninstalling it system-wide is not supported by Podman Desktop as each operating is system is different
  // and we cannot provide a reliable solution to deleting files from /usr/bin/local, etc.
  if (!newValue && isDockerComposeInstalled) {
    await extensionApi.configuration.getConfiguration('compose').update('binary.installComposeSystemWide', true);
    await extensionApi.window.showInformationMessage(
      `Compose binary is already installed system-wide. Try with 'docker-compose' or 'podman compose' commands. If you wish to uninstall it, type 'whereis docker-compose' in a terminal to find its location and delete it.`,
    );
    return;
  }
