...
export async function activate(extensionContext: extensionApi.ExtensionContext): Promise<void> {
  ...
  const onboardingPromptUserForVersionCommand = extensionApi.commands.registerCommand(
    ...
    async () => {
      ...
      // Update the context that we are going to install a custom version so we can show the user the binary version
      // NOTE: There is a BUG with showing the version number in the onboarding sequence. It will show the version
      // of the latest release, not the version that the user has selected. This is because the onboarding sequence
      // is not re-rendered when the context value is updated. This is a known issue and will be fixed in the future.
      // This is why we set it to true, false then true again to force a re-render.
      extensionApi.context.setValue('composeInstallCustomVersion', true, 'onboarding');
      extensionApi.context.setValue('composeInstallCustomVersion', false, 'onboarding');
      extensionApi.context.setValue('composeInstallCustomVersion', true, 'onboarding');
    },
  );
  ...
  // Update the status based upon if the binary is installed SYSTEM WIDE or not
  // we will still show "Setup.." if the user has not installed the binary system-wide
  // as `podman compose` is unable to be used unless it's installed system-wide
  let status: extensionApi.ProviderStatus = 'not-installed';
  if (await handler.isDockerComposeInstalledSystemWide(extensionContext)) {
    status = 'installed';
  }
